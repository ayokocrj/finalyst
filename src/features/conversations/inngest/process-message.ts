...
import { createGenerateExcelModelTool } from './tools/generate-excel';
...
export const processMessage = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Select the AI model dynamically (Kimi if KIMI_API_KEY is configured, else Claude 3.5 Sonnet)
    const kimiApiKey = process.env.KIMI_API_KEY;
    const selectedModel = kimiApiKey
      ? openai({
      ...
      : anthropic({
          ...
          defaultParameters: { temperature: 0, max_tokens: 8000 },
        });

    // Create the financial analyst agent with file tools
    const financialAgent = createAgent({
      name: "finalyst-analyst",
      description: "An elite AI financial analyst for Search Funds",
      system: systemPrompt,
      model: selectedModel,
      tools: [
        ...
        createGenerateExcelModelTool({ internalKey, projectId }),
       ],
    });

    // Create network with single agent
    const network = createNetwork({
      name: "finalyst-network",

        ...
        return await toolStep?.run("read-files", async () => {
          ...
          for (const fileId of fileIds) {
            ...
            if (file) {
              ...
              } else if (file.storageUrl && file.name.toLowerCase().endsWith(".pdf")) {
                try {
                  const pdfText = await extractTextFromPdf(file.storageUrl);
                  results.push({
                    id: file._id,
                    name: file.name,
                    content: pdfText,
                  ...
                  results.push({
                    id: file._id,
                    name: file.name,
                    content: `Error reading PDF file: ${err instanceof Error ? err.message : String(err)}`,
                  });
                }
              }
            }
